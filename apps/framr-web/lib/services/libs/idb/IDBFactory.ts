 ...
 */
export class IDBFactory<T extends DBSchema> {
  /** Opened database */
  ...
  async insert<S extends StoreNames<T>>(
    ...
    try {
      ...
      throw new IDBError((error as Error).message, 'insert');
    }
  }
  ...
  async findOne<S extends StoreNames<T>>(
    storeName: S,
    key: IDBValidKey,
    tx?: IDBTransaction<T, S, 'readonly' | 'readwrite'>
  ...
  async update<S extends StoreNames<T>>(
    ...
    try {
      ...
      throw new IDBError((error as Error).message, 'update');
    }
  }
  ...
  async delete<S extends StoreNames<T>>(
    ...
    try {
      ...
      throw new IDBError((error as Error).message, 'delete');
    }
  }
  ...
  async deleteMany<S extends StoreNames<T>>(
    ...
    try {
      ...
      throw new IDBError((error as Error).message, 'deleteMany');
    }
  }
  ...
  async findManyByIndex<S extends StoreNames<T>>(
    ...
    try {
      ...
      throw new IDBError((error as Error).message, 'findManyByIndex');
    }
  }
  ...
  async count<S extends StoreNames<T>>(
    ...
    try {
      ...
      throw new IDBError((error as Error).message, 'count');
    }
  }
  ...
  async $transaction<M extends IDBTransactionMode, S extends StoreNames<T>>(
    ...
    /**
       ...
       * const transactionCallback: TransactionCallback<DBSchema, 'readwrite'> =
          async (transaction) => {
            const records = IDBFactory.findAll('testStore', transaction);
            if(records.length > 0)
              await IDBFactory.update('testStore', records[0].key, 'tx_value_2', transaction);
            else await IDBFactory.insert(
              'testStore',
              {
                key: 'tx_key_1',
                value: 'tx_value_1',
  ...
  async clear<S extends StoreNames<T>>(storeName: S) {
    try {
      ...
      throw new IDBError((error as Error).message, 'clear');
    }
  }
}
